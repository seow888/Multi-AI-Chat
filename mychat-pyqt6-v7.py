...
class ChatSessionManager:
    ...
    def attach_file(self):
        """FIXED VERSION - Combines v6 reliability with v7 Excel support"""
        file_path, _ = QFileDialog.getOpenFileName()
        if not file_path or not os.path.exists(file_path):
            return

        # Validate file size (100MB limit) ✅ CARRYOVER FROM YOUR CODE
        file_size = os.path.getsize(file_path)
        if file_size > 100 * 1024 * 1024:
            QMessageBox.critical(self, "Error", "File exceeds 100MB size limit")
            return

        # Store path for processing ✅ CRITICAL FOR THREAD SAFETY
        self.attachment_path = file_path  

        # Handle Excel files in background thread ✅ FIX FROM ORIGINAL DIFF
        if file_path.lower().endswith(('xls', 'xlsx')):
            self.executor.submit(
                self._process_excel_file, 
                file_path
            ).add_done_callback(
                lambda future: self._update_ui_with_excel_data(future.result())
            )
        else:
            # Add this to store in session:
            session_id = self.current_session_id  # Assume this exists
            self.attach_file_to_session(session_id, file_path)  # New helper method
            # Existing non-Excel handling ✅ PRESERVE v6 FUNCTIONALITY
            file_name = os.path.basename(file_path)
            self.add_user_message(f"Attached file: {file_name}")
