...
class ChatSessionManager:
            ...
            def process(self):
                try:
                    ...
                    if self.file_name.lower().endswith(('.xls', '.xlsx', '.xlsm')):
                        file_size = os.path.getsize(self.file_path) # Get file size here 
                        content = f"<attach-binary>📎 {self.file_name} ({file_size/1024:.1f} KB)</attach-binary>" # Create binary attachment content 
                        self.finished.emit(self.file_name, content, self.file_path) # Emit finished signal with binary content 
                        return # EARLY RETURN - Skip text processing for Excel for now
                    ...
                    self.finished.emit(self.file_name, content, self.file_path) # Emit filepath as well
    ...
    #        file_size = os.path.getsize(file_path)
    #        content = f"<attach-binary>📎 {file_name} ({file_size/1024:.1f} KB)</attach-binary>"
    #        worker.finished.emit(file_name, content)
    #    except Exception as e:
