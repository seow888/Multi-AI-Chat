...
class ChatSessionManager:
    ...
    def attach_file(self):
        """Enhanced file attachment with thread safety, encoding detection, and MIME/extension checks"""
        ...
        class FileWorker(QObject):
            ...
            def __init__(self, path):
                ...
                self.text_extensions = {'.txt', '.csv', '.log', '.md', '.json', '.xml'}
    
            def detect_text_file(self):
                """Combined MIME type and extension check"""
                mime, _ = mimetypes.guess_type(self.file_path)
                ext = os.path.splitext(self.file_name)[1].lower()
                return (mime and mime.startswith('text/')) or (ext in self.text_extensions)
            ...
            def process(self):
                try:
                    ...
                    elif not self.detect_text_file():
                        with open(self.file_path, 'rb') as f:
                            content = f"<attach-binary>{base64.b64encode(f.read()).decode('utf-8')}</attach-binary>"
                    else:  # Text file processing
                        for encoding in ['utf-8', 'utf-16-le', 'iso-8859-1', 'gbk', 'cp1252']:
                            try:
                                with open(self.file_path, 'r', encoding=encoding) as f:
                                    content = f"<attach-text>{f.read()}</attach-text>"
                                    break
                            ...
                            raise UnicodeError("Failed to decode text file with common encodings")
