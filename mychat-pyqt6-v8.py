...
class ChatSessionManager:
    ...
    def attach_file(self):
        ...
        try:
            # Universal file size check (100MB limit)
            file_size = os.path.getsize(file_path)
            if file_size > 100 * 1024 * 1024:
                self.show_error("File exceeds 100MB size limit")
                return

            # Excel handling with pandas error capture
            if file_path.lower().endswith(('.xls', '.xlsx')):
                try:
                    df = pd.read_excel(file_path)
                    html_content = df.to_html(index=False)
                    self.message_input.append(f"<attach-excel>{html_content}</attach-excel>")
                    self.attachment_label.setText(f"Attached: {os.path.basename(file_path)}")
                except pd.errors.EmptyDataError:
                    self.show_error("Excel file is empty or corrupt")
                except pd.errors.ParserError:
                    self.show_error("Invalid Excel format")
                except Exception as e:
                    self.show_error(f"Excel error: {str(e)}")
            
            # Text files with encoding fallbacks
            else:
                content = None
                for encoding in ['utf-8', 'utf-16', 'latin-1']:  # From class __init__
                    try:
                        ...
                        self.show_error(f"Read error: {str(e)}")
                        return
                
                if content:
                    self.message_input.append(f"<attach-text>{content}</attach-text>")
                    self.attachment_label.setText(f"Attached: {os.path.basename(file_path)}")
                else:
                    self.show_error("Failed to decode text file")

        except Exception as e:
            self.show_error(f"Attachment failed: {str(e)}")
