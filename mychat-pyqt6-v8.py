...
class ChatSessionManager:
    ...
    def attach_file(self):
        """Threaded file attachment with comprehensive error handling"""
            ...
            def process(self):
                try:
                    ...
                    if self.file_path.lower().endswith(('.xls', '.xlsx')):
                        ...
                        if not content:
                            self.error.emit("Failed to decode text file")
                            return
    
                    self.finished.emit(file_name, content)

            except pd.errors.EmptyDataError:
                self.error.emit("Excel file is empty or corrupt")
            except pd.errors.ParserError:
                self.error.emit("Invalid Excel format")
            except Exception as e:
